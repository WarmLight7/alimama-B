...
public:
  ...
  //转换判断类型
    ...
    SearchServiceImpl() {
        ...
        stub_ = SearchService::NewStub(grpc::CreateChannel("node-1:50051", grpc::InsecureChannelCredentials()));
    ...
    Status Search(ServerContext* context, const Request* request, Response* response) override {
        ...
        while(!adGroupPQ.empty())
        {
            ...
            adGroupPQ.pop();
            response->add_adgroup_ids(nowAdgroup.adgroup_id);
            response->add_myprices(nowAdgroup.price);
            response->add_myctrs(nowAdgroup.ctr);
            response->add_myscores(nowAdgroup.score);
        }
